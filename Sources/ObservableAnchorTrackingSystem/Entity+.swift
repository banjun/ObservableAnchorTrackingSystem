...
extension Entity {
    convenience init(anchorTargets: [AnchoringComponent.Target], withDebugAxes: Bool = false) {
        self.init()
        anchorTargets.forEach { t in
            let e = AnchorEntity(t, trackingMode: .predicted)
            e.components.set(ObservableAnchorEntityComponent())
            if withDebugAxes {
                e.addChild(Entity(axesWithLength: 0.02))
            }
            addChild(e)
        }
    }
